...
def complaints():
    ...
    if request.method=="GET":
    	...
    	s.starttls()
    	s.login("178w1a05c9@vrsiddhartha.ac.in", "@Sritar9876")
    	message = " Complaint Id: "+compId+" \n Faculty Id: "+loginid+" \n Name: "+loginname+" \n Department: "+dept+" \n Issue : "+issuedescription+" "
    	print(message)
    	s.sendmail("178w1a05c9@vrsiddhartha.ac.in",compoffemail, message)
    	s.quit()
...
def admindashboard():
	loginid=session['adminid']
	loginname=session['adminname']
	if request.method=="GET":
		cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
		cursor.execute('SELECT * FROM complaintstable')
		data=cursor.fetchall()
		return render_template('adminhome.html',loginid=loginid,loginname=loginname,value=data)
